...
function emptyGameState(name){
  return {
    ...
    // Kullanıcının tahtadan sildiği kelimeleri tekrar otomatik önermemek için
    // oyun bazında tutulur. "Tahtayı Temizle" ile sıfırlanır.
    deletedWords: []
  };
}
...
function loadActiveGameIntoState(){
  ...
  g.deletedWords = Array.isArray(g.deletedWords) ? g.deletedWords : [];
}
...
function persistActiveGame(){
  ...
  g.deletedWords = Array.from(g.deletedWords || []);
...
function getWordsThroughCell(b,r,c){
  const words = [];
  if(!inside(r,c) || !b[r][c]) return words;

  const h = getWordAt(b,r,c,0,1);
  const v = getWordAt(b,r,c,1,0);
  if(h.word.length > 1) words.push(h.word);
  if(v.word.length > 1) words.push(v.word);
  return words;
}
...
function rememberDeletedWords(words){
  ...
  const g = getActiveGame();
  if(!g) return;
  if(!Array.isArray(g.deletedWords)) g.deletedWords = [];

  for(const word of words){
    if(!g.deletedWords.includes(word)) g.deletedWords.push(word);
  }
}
...
function setCellFromKey(key){
  ...
  if(key === "Backspace" || key === "Delete"){
    board[r][c] = null;
    if(c>0) selected.c--;
    // Tahta değiştiği anda eski önerileri ve önizlemeyi geçersiz kıl.
    clearResults(false);
    renderBoard();
    persistActiveGame();
    return;
...
function buildCandidate(word,r,c,dr,dc,rack){
  const g = getActiveGame();
  if(g && Array.isArray(g.deletedWords) && g.deletedWords.includes(word)) return null;
...
function findMoves(){
  ...
  function addStart(starts,r,c){
    if(inside(r,c)) starts.add(keyOf(r,c));
  }
  ...
  for(const word of dictionaryWords){
    ...
    for(const [dr,dc] of [[0,1],[1,0]]){
      ...
      if(empty){
        // İlk hamle: yalnızca merkezi kapsayan başlangıçlar.
        for(let i=0;i<word.length;i++){
          const sr=CENTER-dr*i, sc=CENTER-dc*i;
          addStart(starts,sr,sc);
        }
      }else{
        // Mevcut tahtadaki her taş, hem kesişen hem de paralel temas
        // eden olası başlangıçlar için referans noktasıdır.
        for(let r=0;r<SIZE;r++){
          for(let c=0;c<SIZE;c++){
            ...
            for(let i=0;i<word.length;i++){
              // A) Aynı hücrede kesişme: word[i], mevcut harfle aynı olmalı.
              if(word[i]===existing){
                addStart(starts, r-dr*i, c-dc*i);
              }

              // B) Paralel temas: kelime, mevcut taşın komşu satır/sütununda
              // başlayabilir; kelimedeki harfin mevcut taşla aynı olması gerekmez.
              if(dr===0){
                // Yatay kelime: mevcut taşın üstü veya altı.
                addStart(starts, r-1, c-i);
                addStart(starts, r+1, c-i);
              }else{
                // Dikey kelime: mevcut taşın solu veya sağı.
                addStart(starts, r-i, c-1);
                addStart(starts, r-i, c+1);
              }
            }
          }
